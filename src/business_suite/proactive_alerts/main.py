...
def _load_subagent(name: str):
    ...
    module_name = f"src.business_suite.proactive_alerts.sub_agents.{name}"
...
async def run_proactive_insights_workflow() -> Dict[str, Any]:
    ...
    for r in rules:
        ...
        agent_module = _load_subagent("threshold_analyzer_agent")
        system_prompt = agent_module.SYSTEM_INSTRUCTIONS.format(
            sys_context_json=json.dumps(sys_context, indent=2),
