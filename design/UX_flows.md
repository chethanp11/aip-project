...
This document describes the implemented user flows in `src/ui`, suite micro-frontends, and `/api/v1` routes.

## 1. Shell Authentication Flow

### Entry

User opens `/`. The root shell in `src/ui/index.html` shows the login screen until a valid token exists in local storage.
...
After login, the sidebar drives page switching in `src/ui/index.js`. 
...
Analytics Professional navigation renders six Analyst Actions: PRISM, Research, Explore Data, Build Report, Root Cause Analysis, and Recommend Actions.
...
PRISM is exposed as a standalone Analyst Action product in the shell and reuses the report rationalization micro-frontend.
...
Route: `POST /api/v1/workflows/analyst/research`

Flow:

1. Analyst submits a business question, metric, data element, report lineage topic, or artifact topic.
2. API invokes the reusable `knowledge_retrieval` capability.
3. Workflow returns governed context, match count, and source-family labels.
4. UI renders the research summary and source families in the standalone Research product.
