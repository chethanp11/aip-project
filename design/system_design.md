...
AIP is implemented as a Python FastAPI gateway plus static micro-frontends. The source-of-truth runtime is `src/`; infrastructure and data surfaces live in `Infra/`.
...
src/main.py
  ...
  ├─ mounts /reports from Infra storage
...
AIP uses Infra through configuration and client abstractions only.
...
`src/shared/infra_client/redis_client.py`
...
`src/shared/infra_client/retrieval_client.py`
...
`src/shared/infra_client/storage_client.py`

- Ensures report, artifact, archive, and log directories exist.
- Writes generated content to configured Infra directories.

## 6. KMS Data Model and Retrieval

KMS is implemented in `src/kms/index.py`.
...
| POST | `/api/v1/workflows/analyst/research` | Run standalone Analyst Research against governed knowledge. |
...
- Start app from `start.sh`.
- Use `check.sh` to start/check Docker Compose and connectivity.
- Keep data and generated outputs in `Infra/`.
- Do not store secrets in tracked files; `Infra/secrets` is a placeholder.
...
- Report build sessions are in memory; published HTML persists in `Infra/` reports storage.
