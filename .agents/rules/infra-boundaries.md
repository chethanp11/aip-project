...
This document outlines the strict physical boundaries and data integration models between the application runtime (root directory) and external storage/database layers (`Infra/`).
...
- **Physical Boundary Mapping**:
  - Code/Endpoints: Reside exclusively in `src/`.
  - Seeds/Dynamic files: Reside exclusively in `Infra/storage/` (specifically`storage/reports/`, etc.).
  - Logs: Written exclusively to `Infra/logs/`.
...
- **Database Client Wrappers**:
  - Always leverage the centralized connection client classes under `src.shared.infra_client` instead of writing custom psycopg2, redis, or neo4j connections.
...
- To retrieve grounded metrics context or perform semantic searches:
  ...
  - Do not bypass this route by writing direct file reads or writing custom raw SQL queries directly inside reporting/analytics workflow directories.
